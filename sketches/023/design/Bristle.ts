...
import { BRISTLE_MAX_WEIGHT, BRISTLE_MIN_WEIGHT, LIGHTNESS_VARIANCE } from './constants'

interface BristleConstructor {
  i: number, layerI: number, strokeI: number, bristleSimplex: SimplexNoise, hue: number, lightness: number
}
...
class Bristle {
  ...
  constructor({ i, layerI, strokeI, bristleSimplex, hue, lightness }: BristleConstructor) {
    ...
    this.color = `hsl(${hue}, 100%, ${lightness + bristleSimplex.noise2D(strokeI, i) * LIGHTNESS_VARIANCE}%)`
  }
}
